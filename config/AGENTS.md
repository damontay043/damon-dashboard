...
- [ ] **Cron report QA before relay — MANDATORY CHECKLIST:** Before forwarding ANY cron report to bro, verify: (1) All dates have correct day-of-week (`date -d` check), (2) Source labels preserved (no merging CoinDesk/TheBlock into "Crypto" or CNN/CNBC into "World"), (3) All data dimensions present (if report has 7d/30d/90d, relay must too — don't over-condense), (4) Directional/causal claims are logically sound (e.g., don't claim price drops improve HF when collateral is in crypto), (5) Numbers in relay match source report. If any check fails, fix before sending. The review layer must ADD quality, not strip data. (MISS: Feb 28 — four QA failures in one morning briefing.)
