...
- [ ] **Cron report relay — TWO-VERSION FORMAT:** Send TWO separate messages for review-mode cron reports: (1) **Original cron report** — verbatim from the saved file, adapted for WhatsApp formatting (no markdown tables, use bullet lists). Do NOT rewrite, condense, or restructure. (2) **Damon's review layer** — QA corrections, contextual additions, novel analysis that the cron agent couldn't do. This is ADDITIVE, not a replacement. Bro reads every report and prefers lengthier reports with nuance. The review layer must ADD quality, not strip data. (MISS: Feb 28 — tried to condense wellness report into single message, result was subjectively WORSE than cron original. Bro requested two-version approach.)
